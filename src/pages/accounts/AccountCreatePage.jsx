...
import { createAccount, getAccountsByCustomer, getAccountSubtypes } from '../../api/accountApi';
...
export const AccountCreatePage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [branchesResponse, subtypesResponse] = await Promise.all([
          getAllBranches(),
          getAccountSubtypes(),
        ]);
        ...
        setFormData((prev) => ({
          ...prev,
          accountSubtypeId: prev.accountSubtypeId || String(subtypes[0]?.id || subtypes[0]?.accountSubtypeId || ''),
        }));
        setLoading(false);
      } catch {
        ...
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow">
          ...
          <div className="mb-6">
            ...
            >
              ...
              {accountSubtypes.map((at) => (
                <option key={at.id || at.accountSubtypeId} value={at.id || at.accountSubtypeId}>
                  {at.name || at.description || at.code}
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="mb-6">
            ...
            <p className="text-gray-600 text-sm mt-1">
              El monto se validara contra las reglas del servicio Core.
            </p>
          </div>
